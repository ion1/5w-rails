...
\newcommand{\en}[1]{\pdfforeignlanguage{english}{#1}}
...
Esimerkin alussa oleva \emph{@wip} (\emph{''\en{work in progress}''}) on tagi,
...
Seuraava skenaario (huom. \emph{''\en{Scenario \textbf{Outline}}''})
suoritetaan sen sisältämän \en{Examples}-taulukon mukaisesti neljä kertaa,
...
tietää, mitä tehdä, kun speksissä lukee esim. \emph{''\en{When I press
\dots}''} tai \emph{''\en{Then I should see \dots}''}.

Tämän speksin ainoa testi, jonka toteuttavan koodin joudumme toteuttamaan itse,
on \emph{''\en{Then I should see a timestamp no older/newer than \dots}''}. Se
